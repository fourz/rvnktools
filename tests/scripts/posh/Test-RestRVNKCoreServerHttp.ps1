# Test-RestRVNKCoreServerHttp.ps1
# Tests the HTTP functionality of RVNKCoreServer REST API endpoints
# Requires: PowerShell 7.0 or later

param (
    [Parameter(Mandatory = $false)]
    [string]$BaseUrl = "http://localhost:8080",  # Default HTTP port from api.http.port
    [Parameter(Mandatory = $false)]
...
function Test-HttpConnection {
    ...
    if ($result.Success) {
        Write-TestResult -TestName "HTTP Connection" -Success $true -Message "Successfully connected to HTTP endpoint"
    }
    else {
        Write-TestResult -TestName "HTTP Connection" -Success $false -Message "Failed to connect to HTTP endpoint" -Error $result.Error
    }
}
...
# Run Tests
Write-Host "`nStarting RVNKCoreServer HTTP API Tests`n" -ForegroundColor Cyan
